...
TRAVIS_DEBIAN_NETWORK_ENABLED="${TRAVIS_DEBIAN_NETWORK_ENABLED:-true}"
...
case "${TRAVIS_DEBIAN_EXTRA_REPOSITORY:-}" in
	https:*)
		TRAVIS_DEBIAN_EXTRA_PACKAGES="${TRAVIS_DEBIAN_EXTRA_PACKAGES} apt-transport-https sudo fakeroot fakechroot"
		;;
esac
...
CMD dpkg-checkbuilddeps 2>&1|cut -d: -f4|sudo apt install -y -
CMD ${TRAVIS_DEBIAN_GIT_BUILDPACKAGE} ${TRAVIS_DEBIAN_GIT_BUILDPACKAGE_OPTIONS} --git-builder="debuild -D -i -I" --git-ignore-branch --git-export-dir=${TRAVIS_DEBIAN_BUILD_DIR} -uc -us -sa
EOF
...
docker run --cap-add=SYS_CHROOT --privileged --env=DEB_BUILD_OPTIONS="${DEB_BUILD_OPTIONS:-}" ${ARGS} "${TAG}"
